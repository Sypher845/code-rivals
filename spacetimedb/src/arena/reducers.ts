...
function calculateRoundPoints(
  ...
  if (totalTestcases <= 0n || testcasesPassed <= 0n) {
    return 0n;
  }

  const roundDurationSeconds = getRoundDurationSeconds(roundNumber);
  const boundedTimeTaken =
    timeTakenSeconds > roundDurationSeconds
      ? roundDurationSeconds
      : timeTakenSeconds;
  const secondsRemaining = roundDurationSeconds - boundedTimeTaken;
  const maxPoints = getRoundMaxPoints(roundNumber);
  const accuracyPool = (maxPoints * 70n) / 100n;
  const speedPool = maxPoints - accuracyPool;
  const completionRatioNumerator =
    testcasesPassed > totalTestcases ? totalTestcases : testcasesPassed;

  const accuracyPoints =
    (accuracyPool * completionRatioNumerator) / totalTestcases;
  const speedPoints =
    (((speedPool * completionRatioNumerator) / totalTestcases) *
      secondsRemaining) /
    roundDurationSeconds;
...
export const submit_round_result = spacetimedb.reducer(
  ...
  (
    ...
    const computedPoints = calculateRoundPoints(
      room.currentRound,
      timeTakenSeconds,
      testcasesPassed,
      totalTestcases,
