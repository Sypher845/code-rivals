...
function DescriptionPanel() {
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[var(--ghost-border)] bg-[rgba(10,14,20,0.94)]">
      {/* header */}
      <div className="flex items-center border-b border-[var(--ghost-border)] px-5 py-3">
...
function EditorPanel() {
  ...
  const editorRef = useRef<ReturnType<OnMount>[0] | null>(null);
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[var(--ghost-border)] bg-[rgba(10,14,20,0.94)]">
      {/* header bar */}
      <div className="flex items-center justify-between border-b border-[var(--ghost-border)] px-4 py-0">
...
function TestCasesPanel() {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[var(--ghost-border)] bg-[rgba(10,14,20,0.94)]">
      {/* tab header */}
      <div className="flex items-center border-b border-[var(--ghost-border)] px-4 py-0">
        <div className="flex items-center gap-1">
          {(["testcase", "result"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveMainTab(tab)}
              className={`inline-flex items-center gap-2 px-4 py-3 text-sm font-medium capitalize transition-colors ${activeMainTab === tab
                  ? "border-b-2 border-[var(--primary)] text-[var(--primary)]"
                  : "text-[var(--text-tertiary)] hover:text-[var(--text-secondary)]"
                }`}
            >
              {tab === "testcase" && (
                <span className="flex h-5 w-5 items-center justify-center rounded bg-[rgba(124,216,124,0.14)]">
                  <Check className="h-3 w-3 text-[#7cd87c]" />
                </span>
              )}
              {tab === "result" && (
                <span className="flex h-5 w-5 items-center justify-center rounded bg-[rgba(0,112,255,0.14)]">
                  <Play className="h-3 w-3 text-[var(--tertiary)]" />
                </span>
              )}
            ...
            <div className="mb-4 flex items-center gap-2">
              {PROBLEM.testCases.map((_, i) => (
                <button
                  ...
                  className={`rounded-md px-4 py-2 text-sm font-medium transition ${activeCase === i
                      ? "bg-[rgba(224,141,255,0.1)] text-[var(--primary)]"
...
export function CodingWindowPage() {
  ...
  return (
    ...
    >
      ...
      {/* ═══════════════ TOP NAV BAR ═══════════════ */}
      <nav className="flex h-14 shrink-0 items-center justify-between border-b border-[var(--ghost-border)] bg-[rgba(10,14,20,0.96)] px-4">
        ...
        <div className="flex min-w-0 flex-1 items-center justify-end gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-lg border border-[rgba(224,141,255,0.2)] bg-[rgba(224,141,255,0.06)] px-4 py-2 text-sm font-medium text-[var(--primary)] transition hover:border-[rgba(224,141,255,0.4)] hover:bg-[rgba(224,141,255,0.12)] hover:shadow-[0_0_20px_rgba(224,141,255,0.15)]"
            >
              <Zap className="h-4 w-4" />
              <span className="text-lg">{POWER_CARD.emoji}</span>
              Use Power
            </button>

            {/* tooltip on hover */}
            {showPowerTooltip && (
              <div className="absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2 whitespace-nowrap rounded-md border border-[var(--ghost-border)] bg-[#0d1017] px-3 py-1.5 text-xs font-medium text-[var(--on-background)] shadow-lg">
                {POWER_CARD.name}
                <div className="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-[#0d1017]" />
              </div>
            )}
          </div>
